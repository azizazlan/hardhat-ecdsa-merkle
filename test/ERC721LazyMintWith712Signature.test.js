...
describe('ERC721LazyMintWith712SignatureChecker', function () {
    before(async function () {
        ...
        this.smartwallet = await deploy(
            'SmartWallet',
            this.accounts[1].address,
        );
        ({ chainId: this.chainId } = await ethers.provider.getNetwork());

        expect(await this.smartwallet.owner()).to.be.equal(
            this.accounts[1].address,
        );
    });
    ...
    describe('Mint all elements', function () {
        before(async function () {
            ...
            await this.registry.grantRole(
                await this.registry.MINTER_ROLE(),
                this.smartwallet.address,
            );
        });
